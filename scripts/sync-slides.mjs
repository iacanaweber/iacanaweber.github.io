...
const configPath = join(repoRoot, 'config', 'slides.json');
const generatedDir = join(repoRoot, 'src', 'content', 'resources');
const texBuildRoot = join(tmpdir(), 'slides-build');
      ...
      if (!pptxConverter) {
        ...
        try {
          ...
          if (existsSync(finalPdfPath)) {
            console.warn(
              `[slides:sync] PPTX conversion failed for ${slide.id}; using existing PDF: ${finalPdfPath}`
            );
            const reason = error instanceof Error ? error.message : String(error);
            console.warn(`[slides:sync] Reason: ${reason}`);
            skippedCount += 1;
          } else {
            throw error;
          }
