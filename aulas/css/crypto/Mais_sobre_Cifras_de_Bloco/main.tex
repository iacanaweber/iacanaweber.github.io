...
\begin{itemize}
    ...
    \item Todos esses modos fornecem \textbf{confidencialidade}
    \begin{itemize}
        \item \textit{Autenticidade} e \textit{integridade} (a mensagem realmente veio do remetente original? o texto cifrado foi alterado durante a transmissão?) serão tratadas mais adiante, ao estudarmos MACs
    \end{itemize}
\end{itemize}

\end{frame}

\begin{frame}{Padding (preenchimento de blocos)}

\begin{itemize}
    \item Cifras de bloco operam em blocos de tamanho fixo (ex.: 128 bits no AES)
    \item Quando o último bloco da mensagem não é múltiplo do tamanho do bloco, é necessário preencher (\textit{padding}) os bits restantes
    \begin{itemize}
        \item Relevante principalmente para os modos \textbf{ECB} e \textbf{CBC}
        \item Modos baseados em fluxo (\textbf{OFB}, \textbf{CFB}, \textbf{CTR}) não exigem \textit{padding}, pois operam bit a bit
    \end{itemize}
    \item Esquema mais comum: \textbf{PKCS\#7}
    \begin{itemize}
        \item Se faltam $N$ bytes para completar o bloco, adicionam-se $N$ bytes de valor $N$
        \item Se a mensagem já é múltipla do bloco, adiciona-se um bloco extra inteiramente de \textit{padding}
        \item Ex. (bloco de 8 bytes), mensagem de 5 bytes \texttt{AB CD EF 12 34} $\to$ \texttt{AB CD EF 12 34 03 03 03}
    \end{itemize}
    \item O receptor remove o \textit{padding} após a decriptação, conferindo se o último byte indica corretamente quantos bytes descartar
\end{itemize}
...
\begin{frame}{Exemplo de criptografia de \textit{bitmaps} no modo ECB (1/2)}
...
\begin{frame}{Exemplo de criptografia de \textit{bitmaps} no modo ECB (2/2)}
...
\begin{itemize}
    \small
    \item Utiliza uma cifra de bloco como base para construir uma \textit{cifra de fluxo autossincronizante} (\textit{self-synchronizing})
...
\vspace{-0.5em}
\begin{center}
    \includegraphics[width=0.4\linewidth]{img/ctr.png}
\end{center}
\vspace{-0.5em}
\begin{itemize}
\small
...
% GCM oculto: será apresentado quando MAC for tratado
\iffalse
...
\begin{frame}{Modo Galois Counter (GCM): Diagrama}

\begin{figure}
    \centering
    \includegraphics[width=0.78\linewidth]{img/galoismode.png}
\end{figure}

\end{frame}

\begin{frame}{Modo Galois Counter (GCM): Operações}
...
\textbf{Autenticação} (todas as multiplicações em $GF(2^{128})$)
\begin{itemize}
    \small
    \item Gerar a subchave de autenticação: $H = e_k(0)$
    \item Calcular $g_0 = AAD \times H$
    \item Calcular $g_i = (g_{i-1} \oplus y_i) \times H,\quad 1 \leq i \leq n$
    \item \textit{Tag} final: $T = (g_n \times H) \oplus e_k(CTR_0)$
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{img/double_encryption.png}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{img/meet_in_the_middle.png}
\end{figure}
...
\begin{frame}{Tripla Encriptação}
  \begin{itemize}
...
\begin{frame}{\textit{Key Whitening}}
  \begin{itemize}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.5\linewidth]{img/key_whitening.png}
    \end{figure}
...
\begin{frame}{Lições Aprendidas}
  \begin{itemize}
    ...
    \item O modo ECB simples possui vulnerabilidades de segurança, independentemente de qual \textit{block cipher} seja utilizada
